...
def test_keeps_an_inexact_total_flagged_as_such() -> None:
    view = EmailPageView.from_page(Page(items=(make_email("a"),), total=250, total_is_exact=False))
...
def test_projects_an_empty_page_as_zero_of_zero() -> None:
    view = EmailPageView.from_page(Page(items=(), total=0, total_is_exact=True))
...
def test_serializes_the_totals_alongside_the_items() -> None:
    ...
    payload = json.loads(EmailPageView.from_page(page).model_dump_json())

    assert set(payload) == {"items", "returned", "total", "total_is_exact"}
