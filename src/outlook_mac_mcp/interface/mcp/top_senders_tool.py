...
TOP_SENDERS_DESCRIPTION = (
    ...
    f"is built by scanning the matching emails, at most {SCAN_CEILING:,} per call. The "
    "output carries senders (address, name, count), scanned (emails counted), total (the "
    "exact number of emails matching the filters) and coverage_is_complete. "
...
def _translate(use_case: TopSenders, model: TopSendersInput) -> SenderRankingView:
    with observed_tool_call(TOP_SENDERS_TOOL) as outcome:
        ...
        return SenderRankingView.from_ranking(ranking)
