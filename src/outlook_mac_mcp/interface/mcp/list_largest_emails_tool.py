...
LIST_LARGEST_EMAILS_DESCRIPTION = (
    ...
    "Graph extended property (the message's stored byte size), not an estimate. Graph has "
    "no way to sort by this property, so the ranking is built by scanning the matching "
    f"emails, at most {SCAN_CEILING:,} per call. The output carries items (id, subject, "
    "sender, received_at, size_bytes), scanned (emails checked), total (the exact number "
    "of emails matching the filters) and coverage_is_complete. "
